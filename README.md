...
- $q = 43; Это уже значение у которого есть имя. Вот это имя и есть переменная
- 'qweqwe' это тоже значение и в пчп работает/ дадим имя и это имя будет переменной
- $foo = 33 + 443 - 2; сначало выполнится выражение справа Потом дается этому выражению имя 
- In PHP, a variable is a named container used to store data values.
- обявление переменной и присваивание это одно и тоже
- Программа на пчп ничего не знает о кодировке. Программа на пчп представляет из себя набор байтов и его результат тоже байт
- $a; ему присваивается нулл говорят

* > TYPE
- int, float, string, bool, resource, array,  
- это область определения для значения
- это ответ на вопрос какие значения могут быть у переменной
- у переменных нет никакого типа
- тип несет само значение а не перменная Напрмер 444; это значение несет тип инт ИЛИ 'adasd' это значение несет тип стринг
- 'foo' - это последовательность байтов
- В пчп строки по байтам считает
- операции со сторокой  . -concat  + -addition
- str length  any
- Тип результата операции всегда определяется оператором 
- резултат арифметических операций всегда число
- Оператор точка - конкатенейшн всегда выдает строку

* > функция
- это кусок нашего кода который можем использовать много раз
- это подпрограмма которая принимает на вход параметры и возврашает значение и имеет имя
- богатая библиотека функций
- Владеть стандартной библиотекой это признак хорощего програмиста (по мануалу пчп)
- неявное приведение (кастование) типов это автоматическое
- явное это (int)'22'
пчп работает не со строками а с байтами там кодировки нет в байтах

* > assert(); это функция для модульного тестирования
- если тру то он никак не замечается иначе дает о себе знать

* > три злейших врага програмиста
- игры
- телевизор
- политика

* > три друга 
- терминология програмирования
- теория 
- соощество

* > composer and phing 
- композер инструмент для управление пакетами и их зависимостями
- phing для автоматической сборки нашего приложения (вручную не вводим команды)

* > autoload
- композер исходя из файла композер джейсон скачивает устанавливает пакеты и создает папку вендор а в нем
- фалй autolod загрузчик который акумулирует загрузчики всех пакетов далее нам надо всего лиш подключит 
- этот файл в нашем файле загрузчике

* > файловая система
- в раннем периоде в лентах хранились данные тогда и придуман файл где начинается изакачиваетмя определенный
- кусок данных назвали файлом и 8 первых бита выделили названию файла
- в компютере все в файлах директории нет
- Folder or directory or папка это есть файл в котором хранятся данные о других фалах (спарвочки)
- Есть файлы справочники (директории) которые хранят данные о других таких же файлах каталогах (справочниках)
- Путь к файлку может быть абсолютным или относительным последовательное перечисление этих каталогов до файла есть абсолютный путь
- Когда мы используем какую то программу для работы с файлами (finder - cmd - bash - ...) это програма указывает на определенный файл
- смд не находится в какой то папко он работает с определенным файлом - спарвочником который хранит данные о других файлах
- путь от данной директории до файла это относительный путь
- файлы каталоги разделяются слешом /с/root/var/file.txt

* > URL unified resource locator
- то что мы набираем в браузере это не путь к файлу это адрес ресурса
- Приняли правило что адрес ресурсов в сети надо установит в следующей форме
 1. протокол (http - ftp - ) ://
 2. имя хоста (домейн)
 3. путь к ресурсу

- Далье задача веб сервера найти ресурс который был запрошен клиентом
- веб сервер берет этот ЮРЛ (- адрес ресурса) и наход приложение передает ему адрес ресурса который асоцииоран с фалом на компе
